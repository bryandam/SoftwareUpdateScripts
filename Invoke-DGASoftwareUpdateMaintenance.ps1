...
If (!$DeclineSuperseded -and !$DeclineByTitle -and !$DeclineByPlugins -and !$RunCleanUpWizard -and !$CleanSUGs -and !$CombineSUGs -and !$UpdateADRDeploymentPackages -and !$CleanSources -and !$MaximumUpateRuntime) {
    Add-TextToCMLog $LogFile "You must choose one of the action parameters: DeclineSuperseded, DeclineByTitle, DeclineByPlugins, RunCleanupWizard, CleanSUGs, CombineSUGs, UpdateADRDeploymentPackages, CleanSources, or MaximumUpdateRuntime." $component 2
    Return
}
...
If ($MaximumUpateRuntime){
    If ($MaximumUpateRuntime.GetType() -eq [System.Collections.Hashtable]){
        #Do nothing, the user successfully passed a hashtable.  Good job user ... good job.
    } ElseIf ($MaximumUpateRuntime.GetType() -eq [System.String]){        

        #If the string doesn't contain valid newline characters but does contain semi-colons then swap them.
        If(($MaximumUpateRuntime -notmatch '`n') -and ($MaximumUpateRuntime -match ';')){            
            $MaximumUpateRuntime = $MaximumUpateRuntime.Replace(";","`n")
        }
        Try{            
            $MaximumUpateRuntime = ConvertFrom-StringData $MaximumUpateRuntime
        } Catch {
            ...
            $MaximumUpateRuntime = $null
        }
        
        #If it's a hashtable now, dance with rejoice and let the user know.  Otherwise ... sad face.
        If ($MaximumUpateRuntime.GetType() -eq [System.Collections.Hashtable]){
            Add-TextToCMLog $LogFile "The MaximumUpdateRuntime paramter was converted from a string to a hashtable with $($MaximumUpateRuntime.Count) values." $component 2
        } Else {
            $MaximumUpateRuntime = $null
...
Try {   
    ...
    $ActiveSoftwareUpdatePointName = ((Get-CMSoftwareUpdatePointComponent -SiteCode $SiteCode).Props).Where({$_.PropertyName -eq 'DefaultWSUS'}).Value2
    ...
    $ActiveSoftwareUpdatePointSSL = ($ActiveSoftwareUpdatePoint.Props).Where({$_.PropertyName -eq 'SSLWSUS'}).Value
    $ActiveSoftwareUpdatePointPort = 8530
    If ($ActiveSoftwareUpdatePointSSL){
        $ActiveSoftwareUpdatePointPort = ($ActiveSoftwareUpdatePoint.Props).Where({$_.PropertyName -eq 'WSUSIISSSLPort'}).Value
        ...
        $ActiveSoftwareUpdatePointPort = ($ActiveSoftwareUpdatePoint.Props).Where({$_.PropertyName -eq 'WSUSIISPort'}).Value
...
If ($DeclineSuperseded -or $DeclineByTitle -or $DeclineByPlugins){
    ...
    Try {
        ...
        Add-TextToCMLog $LogFile "If this operation timed out, please decline the superseded updates from the WSUS Console manually." $component 3
        Add-TextToCMLog $LogFile "Error: $($_.Exception.Message)" $component 3
        Add-TextToCMLog $LogFile "$($_.InvocationInfo.PositionMessage)" $component 3
	    Set-Location $OriginalLocation
	    Return
    ...
    $Updates = $Updates.Where({$_.IsDeclined -eq $False})
    ...
    If (!$ExclusionPeriod){
        ...
        If (((Get-CMSoftwareUpdatePointComponent).Props).Where({$_.PropertyName -eq 'Sync Supersedence Mode'}).Value -eq 0){
              $ExclusionPeriod = 0 
        } Else {
            $ExclusionPeriod = ((Get-CMSoftwareUpdatePointComponent).Props).Where({$_.PropertyName -eq 'Sync Supersedence Age'}).Value
        }
    }
    ...
    #If using the built-in logic for declining upates by title.
    If($DeclineByTitle){
...
If ($CleanSUGs){
    ...
    Try {
        ...
        Add-TextToCMLog $LogFile "Failed to get software udate groups." $component 3                    
        Add-TextToCMLog $LogFile "Error: $($_.Exception.Message)" $component 3
        Add-TextToCMLog $LogFile "$($_.InvocationInfo.PositionMessage)" $component 3
    }
        ...
        If($RemoveEmptySUGs){
            If ((Get-CMSoftwareUpdate -UpdateGroup $SoftwareUpdateGroup -Fast).Count -eq 0){
                ...
                Try {
                    ...
                    Add-TextToCMLog $LogFile "Failed to remove the '$($SoftwareUpdateGroup.LocalizedDisplayName)' software udate group." $component 3                    
                    Add-TextToCMLog $LogFile "Error: $($_.Exception.Message)" $component 3
                    Add-TextToCMLog $LogFile "$($_.InvocationInfo.PositionMessage)" $component 3
                }
...
If ($MaximumUpateRuntime){
    ...
    ForEach ($Value in $MaximumUpateRuntime.GetEnumerator()) {
            ...
            Try 
            {
                ...
                ForEach($Folder in (Get-ChildItem $PackageSourcePath -Directory)){
                    ...
                    If (! $ContentSubFolders.Contains($Folder.Name) ){
                        Remove-Item (Join-Path $PackageSourcePath $Folder.Name) -Force -What:$WhatIfPreference
                        Add-TextToCMLog $LogFile "Removing folder $Folder.Name from the deployment package $($UpdateDeploymentPackage.Name)." $component 2
                    }
                }                
            }
            Catch [System.Exception]
